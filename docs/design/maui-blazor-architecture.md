...
- `UI (Blazor)`
  ...
  - ゲーム編集モーダルで、関連リポジトリのローカルブランチ一覧とリモート追跡ブランチ（`origin/*` など）を再読込して表示し、ブランチ切り替え、現在ブランチ起点の新規ブランチ作成を実行できる
  - ブランチ再読込時は `fetch --all --prune` を実行し、`refs/heads` と `refs/remotes` を収集する（`*/HEAD` は除外）
  - ドロップダウンでリモート追跡ブランチを選択して切り替える場合、対応するローカルブランチが未作成なら `checkout -b <local> --track <remote>` で作成・切り替えを行う
...
- ブランチ操作: ゲーム編集モーダルでローカル/リモート追跡ブランチ一覧再読込、切り替え（必要時は追跡ブランチ作成）、新規作成（作成後チェックアウト）の成功/失敗通知
...
- 実装済み
  ...
  - ゲーム編集モーダルにブランチ操作導線を追加（ローカル + リモート追跡ブランチドロップダウン、切り替え、作成して切り替え）
