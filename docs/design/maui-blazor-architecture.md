...
- `UI (Blazor)`
  ...
  - ゲーム編集モーダルで、関連リポジトリのローカルブランチ一覧取得、ブランチ切り替え、現在ブランチ起点の新規ブランチ作成を実行できる
...
- ブランチ操作: ゲーム編集モーダルでブランチ一覧再読込、切り替え、新規作成（作成後チェックアウト）の成功/失敗通知
...
- 実装済み
  ...
  - ゲーム編集モーダルにブランチ操作導線を追加（ローカルブランチドロップダウン、切り替え、作成して切り替え）
